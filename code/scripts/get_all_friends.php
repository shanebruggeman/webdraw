<?php 
	...
	$query = $db->prepare('select user.id, user.username, user.profile_pic_id from((select requester_id as id from friend_request where requestee_id = :userid and acceptance = true) union (select requestee_id from friend_request where requester_id = :userid and acceptance = true) order by id asc) as friends join user on user.id = friends.id');
