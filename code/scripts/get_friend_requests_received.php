<?php 
	...
	$query = $db->prepare('select user.id, user.username, user.profile_pic_id from (select requester_id as id from friend_request where requestee_id = :userid and acceptance = false) as almost_friends join user on user.id = almost_friends.id');
